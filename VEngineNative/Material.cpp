...
void Material::updateBuffer() {
    ...
    if (diffuseColorTex != nullptr) {
        descriptorSet.bindImageViewSampler(4, *diffuseColorTex);
    }
    else {
    ...
    if (normalTex != nullptr) {
        descriptorSet.bindImageViewSampler(5, *normalTex);
    }
    else {
    ...
    //####//
    if (bumpTex != nullptr) {
        descriptorSet.bindImageViewSampler(6, *bumpTex);
    }
    else {
        descriptorSet.bindImageViewSampler(6, *Application::instance->dummyTexture);
    }
    //####//
    if (roughnessTex != nullptr) {
        descriptorSet.bindImageViewSampler(7, *roughnessTex);
    }
    else {
        descriptorSet.bindImageViewSampler(7, *Application::instance->dummyTexture);
    }
    //####//
    if (metalnessTex != nullptr) {
        descriptorSet.bindImageViewSampler(8, *metalnessTex);
    }
    else {
        descriptorSet.bindImageViewSampler(8, *Application::instance->dummyTexture);
    }
    //####//
    descriptorSet.bindUniformBuffer(3, *materialBuffer);
