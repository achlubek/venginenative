...
float getCloudsAO(vec3 dir, float dirmult){
	...
	return 1.0 - clamp(sumao / 14.0, 0.0, 1.0);
}
...
float visibility(vec3 dir1, vec3 dir2, float d1, float d2){
	...
	for(int i=0;i<8;i++){
		...
		float dx = textureLod(mainPassTex, md, 0.0).g;
		if(dx < dd) return 0.0;
		iter += stepsize;
	}
	return v;
...
vec3 getCloudsAL(vec3 dir){
	...
	for(int i=0;i<32;i++){
		...
		float z = rand2s(UV * r);
		r += 1.1231255;
		vec3 px = normalize( normalize(vec3(x, y, z) * 2.0 - 1.0));
		float mult = mix(sqrt(dot(dayData.sunDir, dir) * 0.5 + 0.5), 1.0, vdt);
		
		vec3 p = point + px * 5010.0;
		vec3 newdir = normalize(p - c);
		float v = visibility(dir, newdir, asd.g, textureLod(mainPassTex, newdir, 0.0).g);
		sum += textureLod(atmScattTex, px, 0.0).rgb * v * mult; 
	}
	
	float dao = (1.0 -  getCloudsAO(dir, 1.0 - vdt)) * pow(vdt, 2.0) * 3.0;
	float daox = ( getCloudsAO(dir, 1.0  ) * 2.0 - 1.0)   * pow(vdt * 0.8 + 0.2, 2.0) ;
	float sao =  (mix(1.0, getCloudsAO(dir, 0.0), (1.0 - pow(1.0 - max(0.0, dot(dir, VECTOR_UP)), 16.0))) * (1.0 - pow(1.0 - vdt, 8.0)) * pow(vdt * 0.97 + 0.03, 1.0) * 14.0) / ((CloudsCeil - CloudsFloor) * 0.0005 + 1.0);
	
	//float dshadow = getAODIR(point, px, 113.0);
			
	float coverage =  smoothstep(0.464, 0.6, CloudsThresholdLow);
//	return vec3(sao) ;
	return clamp(sum / 32.0 , 0.0, 111.0) * coverage + sao + daox * 0.1 ;
}
...
vec2 internalmarchconservative(vec3 p1, vec3 p2){
	...
	if(coverageinv > 0.99) depr = distance(CAMERA, (p1 + p2) * 0.5);
...
float shadows(){
   ...
   // hash1x = rand2s(UV * vec2(Time, Time));
    vec3 hitman = CAMERA + viewdir * data.g;
