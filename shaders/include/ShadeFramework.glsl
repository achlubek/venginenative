...
vec3 shade_ray(vec3 albedo, vec3 normal, vec3 viewdir, float roughness, float metalness, vec3 lightdir, vec3 lightcolor){
    ...
    vec3 spec_color_nonmetal = lightcolor * fresnel_effect(vec3(0.04), roughness, dotNV) * LightingFuncGGX_REF(normal, -viewdir, lightdir, roughness * 0.99 + 0.01, vec3(1.0));
    vec3 spec_color_metal = lightcolor * fresnel_effect(albedo, roughness, dotNV) * LightingFuncGGX_REF(normal, -viewdir, lightdir, roughness * 0.99 + 0.01, albedo);
    vec3 diffuse_color_nonmetal = lightcolor * LightingFuncGGX_REF(normal, -viewdir, lightdir, 1.0, albedo)* (1.0 - fresnel_effect(vec3(0.04), roughness, dotNV) );
