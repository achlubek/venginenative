...
float heightwaterX(vec2 pos, int s){
    pos *= 0.049;
...
float heightwaterZ(vec2 pos, int s){
    ...
    for(int i=0;i<s;i++){   
        ...
        res += wz * sin(snoise2d(pos + t.yx) * 2.0 + T * 3.0 + pos.x + pos.y);
        w += wz;
        wz *= 0.4;
        pos *= 1.9;
        tmod *= 1.8;
...
vec3 cloudsbydir(vec3 dir){
    ...
    vec3 sunx = sun(dir, normalize(SunDirection),  1.0 ,  roughness) * vec3(atmcolor) * 40.0 * shadowwater;
